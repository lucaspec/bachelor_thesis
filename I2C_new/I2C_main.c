...
#define SLAVE_ADDR  0x47 //optical: 0x47, gauge: 0x36
...
uint8_t i2c_write_reg_2B [2] = {0xCE, 0x10};
uint8_t i2c_read_reg_2B [2];
...
*/
    ...
    I2C_Master_WriteReg(SLAVE_ADDR, 0x01, i2c_write_reg_2B, 2);   // write configuration to config register
    ...
    while(1){
        ...
        I2C_Master_ReadReg(SLAVE_ADDR, 0x01, 2);
        //CopyArray(ReceiveBuffer, i2c_read_reg_2B, 2);

        // check status of conversion ready bit
        if(i2c_read_reg_2B[1] & 0x80){

            //while (UCB0CTL1 & UCTXSTP);
            I2C_Master_ReadReg(SLAVE_ADDR, 0x00, 2);   // read measurements from results register
            //CopyArray(ReceiveBuffer, i2c_read_reg_2B, 2);

            // convert to lux
            uint16_t input = concatenate(i2c_read_reg_2B[0], i2c_read_reg_2B[1]);
