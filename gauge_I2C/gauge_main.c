...
int main(void) {
    ...
    // Reset
    //I2C_Master_WriteReg(SLAVE_ADDR, 0x60, Reset, 2);

    // Quick start
    /*while(Data[0] & FSTAT[1] & 0x01) { // Poll MiscCFG.QS(0x0400) and FSTAT.DNR until they become 0 to confirm Quickstart is finished
        I2C_Master_ReadReg(SLAVE_ADDR, 0x2B, 2);
        CopyArray(ReceiveBuffer, Data, 2);
        Data[1] |= 0x04;
        I2C_Master_WriteReg(SLAVE_ADDR, 0xDB, Write3, 2);
        I2C_Master_ReadReg(SLAVE_ADDR, 0x3D, 2);
        CopyArray(ReceiveBuffer, FSTAT, 2);
